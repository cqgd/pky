...
\begin{equation}
y = \beta \exp{- \abs{\omega * d}}
\end{equation}

where $y$ is the contribution of a specific peak to the adjusted readcount we observe, \beta represents peak height and $d$ the distance from the center of the peak in bp.


We can look at several isolated peaks to see what would be an appropriate choice of omega:
...
nls(residual~dist_exp(distance_from_peak,strength,omega_power), 
    data=single_peak, 
    start=list(strength=5, omega_power=5))

#plot this
```


DO FS FIRSt BECAUSE THEN YOU HAVE THE BAITS
